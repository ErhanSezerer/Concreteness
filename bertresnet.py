...
def train_avgbertresnet(num_epochs, model_resnet, model_bert, train_iter, dev_iter, train_iter_bert, dev_iter_bert,
	...
	for epoch in range(num_epochs):
		...
		for (inputs, labels, paths), batch_ids in tqdm(zip(train_iter, train_iter_bert)):
			...
			#run bert
			scheduler_bert = get_linear_schedule_with_warmup(optimizer_bert, num_warmup_steps=0, num_training_steps=total_iter)
        ...
        #validation
		acc, f1, recall, prec, f1_ave, recall_ave, prec_ave, valid_loss = eval_avgbertresnet(dev_iter, dev_iter_bert, model_resnet, model_bert, loss_fn, device)
...
def save_model(model_resnet, model_bert, checkpoint_dir_resnet, checkpoint_dir_bert):
	checkpoint_dir_resnet = os.path.join(checkpoint_dir_resnet, '{model_name}_epochs_{epoch}_lr_{lr}.bin'.format(model_name='resnet', epoch=args.epochs, lr=args.lr))
